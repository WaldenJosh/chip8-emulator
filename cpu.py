...
class CPU:
    ...
    def execute_opcode(self, opcode):
        ...
        if first_nibble == 0x0000:
            ...
            x = (opcode & 0x0F00) >> 8
            kk = opcode & 0x00FF
            log_debug(f"3Xnn: Checking V{x} = {self.v[x]:02X} against kk = {kk:02X}")
            if self.v[x] == kk:
                self.pc += 2
                pc_changed = True
                log_debug(f"3Xnn: Skipping next instruction, PC = {self.pc:04X}")
        ...
        # DXYN: Display n-byte sprite at memory location I at (Vx, Vy), set VF = collision
            ...
            if (opcode & 0x00FF) == 0x9E:
                pass  # TODO: Implement the key handling
